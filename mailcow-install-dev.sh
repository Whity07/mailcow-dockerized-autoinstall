...
# Mailcow autoinstaller script
# Created by Quint Burkley (@qwow5)

UMASK="0022"

if (($UMASK != 0022 )); then
  echo "ERR: umask is not 0022!"
  exit 1
fi

echo "Press enter to confirm the detected value '[value]' where applicable or enter a custom value."
while [ -z "${MAILCOW_HOSTNAME}" ]; do
  read -p "Mailcow Hostname (FQDN): " -e MAILCOW_HOSTNAME
...
ADDITIONAL_SAN=www.${MAILCOW_HOSTNAME},autodiscover.${MAILCOW_HOSTNAME},autoconfig.${MAILCOW_HOSTNAME}
